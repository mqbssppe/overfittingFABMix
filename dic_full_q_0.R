...
# outputDirectory should be given
...
#	dic_starMAP <- -6*cll + 4*logL.theta_map
	...
	Km <- kSelected
	aic <- -2*aic/m + 2*(Km*( 2*p ) + Km - 1 ) 
	bic <- -2*bic/m + log(n)*(Km*( 2*p ) + Km - 1 )
	aic_MAX <- -2*maxL + 2*(Km*( 2*p ) + Km - 1 ) 
	bic_MAX <- -2*maxL + log(n)*(Km*( 2*p ) + Km - 1 )
...
dics0 <- getStuffForDIC(outputDirectory)
