dir.create("simulation_scenario2")
setwd("simulation_scenario2")
...
loading_means <- c(-30, -20, -10, 10, 20, 30, 1, 40)	# distinct factor loadings values
loading_sd <- abs(loading_means)*0.2 + 1		# standard deviations from `loading_means`
...
for(q in qRange){
	set.seed(1)
		outputFolder <- paste0("fabMix_scenario2_sameSigma_q_",q)
	...
	set.seed(1)
		outputFolder <- paste0("fabMix_scenario2_diffSigma_q_",q)
...
for(q in qRange){
        outputFolder1 <- paste0("fabMix_scenario2_sameSigma_q_", q)
        outputFolder2 <- paste0("fabMix_scenario2_diffSigma_q_", q)
...
zFABMIX <- as.numeric(read.table(paste0("fabMix_scenario2_",model_parameterization,"_q_",nFactors,"/singleBestClusterings.txt"), header=TRUE)[1,])
...
# Estimated AdjustedRandIndex and Number of clusters as reported in Table 2-scenario2 of the manuscript:

scenario2_result <- array(data = NA, dim = c(3,2))
colnames(scenario2_result) <- c("estimated_number_of_clusters", "adjusted_rand_index")
rownames(scenario2_result) <- c("fabMix", "EMMIXmfa", "pgmm")

scenario2_result[1, ] <- c(length(table(zFABMIX)), adjustedRandIndex(syntheticDataset$class, zFABMIX))
#scenario2_result[2, ] <- c(length(table(EmMfaZ)), adjustedRandIndex(syntheticDataset$class, EmMfaZ))
scenario2_result[3, ] <- c(length(table(z_pgmm[,1])), adjustedRandIndex(syntheticDataset$class, z_pgmm[,1]))

print(scenario2_result)
