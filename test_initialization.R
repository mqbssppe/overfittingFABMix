...
heated_chains <- function(dirPriorAlphas, rawData, outDir, Kmax, mCycles, burnCycles, g, h, alpha_sigma, beta_sigma, q, normalize, thinning, zStart, nIterPerCycle, initializationScheme){
	if(missing(Kmax)){Kmax <- 20}
	if( missing(initializationScheme) ){ initializationScheme = TRUE }
...
#		dirPriorAlphas <- seq(1,10,length = 8)/Kmax
	...
	if( initializationScheme == TRUE ){
		...
		foreach(myChain=1:nChains, .export=ls(envir=globalenv()) ) %dopar% {
			truncatedDirichletProcess(q = q, originalX = originalX, x_data = x_data, outputDirectory = outputDirs[myChain], 
				Kmax = Kmax, m = 2, thinning = 1, burn = 1, alpha_prior= rep(dirPriorAlphas[myChain], Kmax), g = g, h = h, 
		...
		#	initialization
		iteration <- 1
		cat(paste('-    (1) Initializing from random starting values... '))
		foreach(myChain=1:nChains, .export=ls(envir=globalenv()) ) %dopar% {
			truncatedDirichletProcess(q = q, originalX = originalX, x_data = x_data, outputDirectory = outputDirs[myChain], 
				Kmax = Kmax, m = 2, thinning = 1, burn = 1, alpha_prior= rep( dirPriorAlphas[myChain], Kmax), g = g, h = h, 
				alpha_sigma = alpha_sigma, beta_sigma = beta_sigma, progressGraphs = FALSE, start_values = FALSE, gibbs_z = 0.05)
