...
update_z3 <- function(w, mu, Lambda, y, SigmaINV, K){
	...
	for(k in 1:K){
		...
		probs[index2,k] <- log(w[k]) -0.5*apply(center_x,1,function(tmp){return( as.numeric(t(tmp) %*% SigmaINV[k,,] %*% tmp) )}) + 0.5*sum(log(diag(SigmaINV[k,,]))) 
...
update_SigmaINV <- function(z, y, Lambda, mu, K, alpha_sigma, beta_sigma){
	...
	for(k in 1:K){
		...
		#s <- array(0, dim = c(p,p))
		for(i in index){
			tmp <- x_data[i, ] - (mu[k,] + array(Lambda[k,,],dim = c(p,q)) %*% y[i,])
...
truncatedDirichletProcess <- function(x_data, originalX, outputDirectory, Kmax, m, thinning, burn, g, h, alpha_prior, alpha_sigma, beta_sigma, progressGraphs, start_values, q, gibbs_z){
...
#	print("here")
	#############################################
	#
	OmegaINV.constant <- array(data = 0, dim = c(q,q)); diag(OmegaINV.constant) <- rep(g/h,q)
...
#	print(OmegaINV.constant)
	for (iter in 2:m){
		...
		if(u < gibbs_z){
			f2 <- update_z(w = w.values, mu = array(mu.values,dim = c(K,p)), 
				Lambda = array(Lambda.values,dim = c(K,p,q)), y = y, 
					SigmaINV =array(SigmaINV.values,dim = c(K,p,p)), K = K)
		}else{
			f2 <- update_z2(w = w.values, mu = array(mu.values,dim = c(K,p)), 
					Lambda = array(Lambda.values,dim = c(K,p,q)), 
						SigmaINV =array(SigmaINV.values,dim = c(K,p,p)), K = K)
		}
		z <- f2$z
		kValues[iter] <- length(table(z))
...
#		for(i in 1:n){
#			upd <- update_delta_y_i(x = x_data[i,], mu = mu.values[z[i], ] , SigmaINV = SigmaINV.values[z[i], , ], Lambda = Lambda.values[z[i], , ])
#			y[i,] <- upd$y
			#delta[i, ] <- upd$delta
#		}
...
#		dirPriorAlphas <- seq(1,10,length = 8)/Kmax
	...
	cat(paste0("a = ", dirPriorAlphas[1], ", p = ", p, ", q = ", q, ", n = ",n,", g = ", g, ", h = ", h, ", alpha_sigma = ", alpha_sigma, ", beta_sigma = ", beta_sigma,"\n"))
	#	initialization
	iteration <- 1
...
#		if( 0 < 1 ){
	...
	write.table(file = 'kValues.txt', kValues[(burnCycles+1):mCycles, ], quote = FALSE, row.names = FALSE, col.names = dirPriorAlphas)
	setwd("../")
}
