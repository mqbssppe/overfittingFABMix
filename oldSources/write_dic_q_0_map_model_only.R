...
#	dic_starMAP <- -6*cll + 4*logL.theta_map
	...
	aic <- -2*aic/m + 2*(kSelected*( p + 0 ) + kSelected*p + kSelected - 1 ) 
	bic <- -2*bic/m + log(n)*(kSelected*( p + 0 ) + kSelected*p + kSelected - 1 )
	aic_MAX <- -2*maxL + 2*(kSelected*( p + 0 ) + kSelected*p + kSelected - 1 ) 
	bic_MAX <- -2*maxL + log(n)*(kSelected*( p + 0 ) + kSelected*p + kSelected - 1 )
