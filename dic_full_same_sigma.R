...
dics <- array(data = NA, dim = c(Q, 9))
colnames(dics) <- c('AIC', 'BIC', 'DIC', 'DIC*', 'DIC_map', 'DIC*_map', 'AIC_map', 'BIC_map', 'logL')
for(q in 1:Q){
...
write.table(dics, file = "dics_full_same_sigma.txt", row.names = 1:Q)
...
legend('topleft', colnames(dics[,subIndex]), pch = myPCH[subIndex], col = myCol[subIndex], lwd = 2)
#dev.off()
...
#dics <- read.table("n1000_K4_q4_scenariob_0.2_longer/dics_same_sigma_q_1.txt",header = TRUE); for(q in 2:10){dics <- rbind(dics, read.table(paste0("n1000_K4_q4_scenariob_0.2_longer/dics_same_sigma_q_",q,".txt"),header = TRUE) )}
