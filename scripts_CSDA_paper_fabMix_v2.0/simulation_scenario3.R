dir.create("simulation_scenario3")
setwd("simulation_scenario3")
...
#	simulate data as described in the Appendix A.2
loading_means = 1					# distinct factor loadings values
loading_sd = rep(0.01, length(loading_means))		# standard deviations from `loading_means`
sINV_diag = rep(10,p)	 				# diagonal of inverse variance of errors
set.seed(10)
syntheticDataset <- simData(K.true = K, n = n, q = q, p = p, sINV_values = sINV_diag, loading_means = loading_means, loading_sd = loading_sd )

qRange <- 1:4	# range of values for the number of factors
...
for(q in qRange){
	set.seed(1)
		outputFolder <- paste0("fabMix_scenario3_sameSigma_q_",q)
	...
	set.seed(1)
		outputFolder <- paste0("fabMix_scenario3_diffSigma_q_",q)
...
for(q in qRange){
        outputFolder1 <- paste0("fabMix_scenario3_sameSigma_q_", q)
        outputFolder2 <- paste0("fabMix_scenario3_diffSigma_q_", q)
...
zFABMIX <- as.numeric(read.table(paste0("fabMix_scenario3_",model_parameterization,"_q_",nFactors,"/singleBestClusterings.txt"), header=TRUE)[1,])
...
K1 <- 1
K2 <- K + 5	#max number of components
KRange <- K1:K2
myDir <- "pgmm_scenario3"
dir.create(myDir)
setwd(myDir)
nRandomRuns <- 5
...
# Estimated AdjustedRandIndex and Number of clusters as reported in Table 2-scenario3 of the manuscript:

scenario3_result <- array(data = NA, dim = c(3,2))
colnames(scenario3_result) <- c("estimated_number_of_clusters", "adjusted_rand_index")
rownames(scenario3_result) <- c("fabMix", "EMMIXmfa", "pgmm")

scenario3_result[1, ] <- c(length(table(zFABMIX)), adjustedRandIndex(syntheticDataset$class, zFABMIX))
#scenario3_result[2, ] <- c(length(table(EmMfaZ)), adjustedRandIndex(syntheticDataset$class, EmMfaZ))
scenario3_result[3, ] <- c(length(table(z_pgmm[,1])), adjustedRandIndex(syntheticDataset$class, z_pgmm[,1]))

print(scenario3_result)
